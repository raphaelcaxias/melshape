...
class CheckinRenderer:
    ...
    def _render_contexto_bloco(self, data: CheckinData, health_mode: str) -> CheckinData:
        """Renderiza bloco de contexto específico do pilar."""
        if health_mode == "glp1":
            label = "💉 Algo sobre o tratamento hoje?"
            placeholder = "Sintomas, dose, sensações..."
            key = "ci_glp1_ctx"
        elif health_mode == "bariatric":
            label = "🔪 Como está a alimentação hoje?"
            placeholder = "Volume, tolerâncias, dificuldades..."
            key = "ci_bar_ctx"
        else:
            label = "Dificuldade ou observação (opcional)"
