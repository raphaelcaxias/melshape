...
class ExecutiveDashboardRenderer:
    ...
    def _render_metricas_geral(self, stats: ExecutiveStats) -> None:
        ...
        with col2:
            ...
            metric_card(
                f"{stats.pacientes_ativos} ({pct_ativos}%)",
                "Pacientes Ativos (7d)",
                "✅",
                cor,
            )
        ...
        with col3:
            ...
            metric_card(f"{stats.aderencia_media:.0f}%", "Aderência Média", "📋", cor)
    ...
    @st.cache_data(ttl=60)
    def _query(_self, tabela: str, colunas: str, limit: int = LIMIT_STATS) -> List[Dict]:
