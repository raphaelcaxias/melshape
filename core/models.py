...
class Professional(SerializableMixin):
    ...
    def effective_clinic_id(self) -> str:
        """
        Identificador efetivo da clínica para isolamento de dados.

        Quando clinic_id não está preenchido (profissional solo), o
        próprio email do profissional funciona como workspace de 1 pessoa.
        Quando preenchido, agrupa múltiplos profissionais sob a mesma clínica.

        Sprint 4 — Multitenancy básico (Constituição Cap. VIII: MVP acima
        da perfeição, sem comprometer integridade arquitetural).
        """
        return self.clinic_id or self.email
