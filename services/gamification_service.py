...
class GamificationService:
    ...
    def _process_achievement_checks(self, checks: list[tuple[str, str, bool]]) -> list[Achievement]:
        ...
        # Sprint 5 — Otimização N+1: pré-busca conquistas já desbloqueadas em 1 query
        # ao invés de verificar existência individualmente para cada check
        ...
        try:
            if self.db.is_real and self.db.client:
                ...
                if uid:
                    r = (
                        self.db.client
                        .table("badges_usuario")
                        .select("badges(nome)")
                        .eq("perfil_id", uid)
                        .execute()
                    )
                    for row in (r.data or []):
                        badge = row.get("badges") or {}
                        nome = badge.get("nome") if isinstance(badge, dict) else None
                        if nome:
                            existing_names.add(nome)
        except Exception:
            pass  # se falhar, cai no caminho individual (unlock_achievement faz a verificação)

        for name, title, condition in checks:
            if not condition:
                continue

            # Pula se já desbloqueada (verificação em memória — zero query adicional)
