...
class ClinicalLoopService:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # PATIENT EVENT LOOP — chamado pelo Orchestrator após cada evento
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def after_event(
        ...
        """
        Avalia alertas clínicos automáticos após um evento do paciente.

        Chamado pelo Orchestrator._cascade() após todo evento.
        Cria alertas in-app para o profissional quando necessário.

        Regras (sem spam — máx. 1 alerta por tipo por dia):
        - Humor <= 2 por 3 dias seguidos → alerta de risco emocional
        - Streak == 0 e era >= 7 → alerta de recaída
        - Peso subiu >= 2kg em 7 dias → alerta de ganho rápido
        - Energia <= 2 por 3 dias seguidos → alerta de fadiga
        """
        if not patient_id:
            return

        try:
            self._check_mood_risk(patient_id, user)
            self._check_relapse(patient_id, user, result)
            self._check_weight_spike(patient_id)
        except Exception as e:
            logger.warning(f"after_event: {e}")

    def _check_mood_risk(self, patient_id: str, user: dict) -> None:
        """Alerta se humor <= 2 por 3+ dias."""
        if not (self.db.is_real and self.db.client):
            return
        try:
            from datetime import date, timedelta
            tres_dias = [(date.today() - timedelta(days=i)).isoformat() for i in range(3)]
            r = (
                self.db.client.table("checkins")
                .select("humor, data_checkin")
                .eq("perfil_id", patient_id)
                .in_("data_checkin", tres_dias)
                .execute()
            )
            registros = r.data or []
            if len(registros) >= 3 and all(int(c.get("humor", 5)) <= 2 for c in registros):
                nome = user.get("name", "Paciente")
                self._criar_alerta_pro(
                    patient_id,
                    titulo=f"⚠️ {nome}: humor baixo 3 dias seguidos",
                    descricao="Paciente registrou humor ≤ 2 por 3 dias consecutivos. Considere entrar em contato.",
                    gravidade=2,
                    tipo="humor_baixo",
                )
        except Exception as e:
            logger.debug(f"_check_mood_risk: {e}")

    def _check_relapse(self, patient_id: str, user: dict, result: Any) -> None:
        """Alerta se paciente perdeu streak >= 7."""
        try:
            streak_atual = getattr(result, "streak", 0) or 0
            if streak_atual == 0:
                # Busca histórico para ver se tinha streak alto
                melhor = getattr(result, "best_streak", 0) or 0
                if melhor >= 7:
                    nome = user.get("name", "Paciente")
                    self._criar_alerta_pro(
                        patient_id,
                        titulo=f"😔 {nome}: sequência de {melhor} dias interrompida",
                        descricao=f"Paciente perdeu uma sequência de {melhor} dias. Momento ideal para contato.",
                        gravidade=2,
                        tipo="recaida",
                    )
        except Exception as e:
            logger.debug(f"_check_relapse: {e}")

    def _check_weight_spike(self, patient_id: str) -> None:
        """Alerta se peso subiu >= 2kg em 7 dias."""
        if not (self.db.is_real and self.db.client):
            return
        try:
            r = (
            ...
            pesos = r.data or []
            if len(pesos) >= 2:
                ultimo = float(pesos[0].get("peso", 0))
                penultimo = float(pesos[-1].get("peso", 0))
                if ultimo - penultimo >= 2.0:
                    self._criar_alerta_pro(
                        patient_id,
                        titulo=f"⚖️ Ganho de {ultimo - penultimo:.1f}kg nos últimos registros",
                        descricao=f"Peso subiu de {penultimo:.1f}kg para {ultimo:.1f}kg. Verificar.",
                        gravidade=1,
                        tipo="ganho_peso",
                    )
        except Exception as e:
            logger.debug(f"_check_weight_spike: {e}")

    def _criar_alerta_pro(
        self,
        patient_id: str,
        titulo: str,
        descricao: str,
        ...
        """Cria alerta clínico no banco — sem duplicatas no mesmo dia."""
        if not (self.db.is_real and self.db.client):
            return
        try:
            from datetime import date
            hoje = date.today().isoformat()
            # Evita duplicata: mesmo tipo + paciente + hoje
            existe = (
            ...
            self.db.client.table("alertas_clinicos").insert({
            ...
            logger.info(f"🏥 Alerta clínico criado: {titulo}")
        except Exception as e:
            logger.debug(f"_criar_alerta_pro: {e}")
