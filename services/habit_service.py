...
from core.habit_repository import Habit, HabitRecord
...
_XP_PER_HABIT: int = config.XP_HABITO if hasattr(config, "XP_HABITO") else 15
...
class HabitService:
    ...
    def _calculate_streak_from_dates(self, dates: list[date]) -> int:
        """Delegado para streak_utils.calculate_streak (módulo canônico)."""
        from services.streak_utils import calculate_streak
