"""
Melshape — Banco de alimentos: Supabase + 55 alimentos brasileiros no fallback.
Busca com normalização de acentos e ordenação por frequência.
"""
import logging
...
def _normalize(text: str) -> str:
    """Remove acentos e converte para minúsculo para busca."""
    import unicodedata
...
class FoodService:
    ...
    def get_foods_by_category(self, category_code: str) -> List[Dict[str, Any]]:
        if self.use_supabase:
            try:
                ...
                logger.warning(f"Supabase get_foods_by_category: {e}")
    ...
    def search_foods(self, term: str,
        ...
        """
        Busca com normalização de acentos e ordenação por frequência de uso.
        """
        if self.use_supabase and term:
            try:
                ...
                logger.warning(f"Supabase search_foods: {e}")
