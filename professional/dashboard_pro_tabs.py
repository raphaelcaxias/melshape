...
class DashboardTabsRenderer:
    ...
    def render_alertas(self) -> None:
        ...
        try:
            alertas = self._query(
                "vw_alertas_abertos",
                "perfil_id,nome_completo,email,titulo,descricao,gravidade,criado_em",
            )
    ...
    def _render_alerta_item(self, alerta: Dict) -> None:
        ...
        try:
            ...
            perfil_id = alerta.get("perfil_id", "")
            email = alerta.get("email", "")
            
            self._render_card_alerta(nome, titulo, descricao, data, cor, perfil_id, email)
        except Exception as e:
    ...
    def _render_card_alerta(
        ...
        """Renderiza card de alerta com ação direta de notificar paciente.

        Fecha o último elo da cadeia identificado na Auditoria Mestra
        (Auditoria 13): 'Profissional vê alerta → Profissional age ❌'.
        """
        desc_html = (
        ...
        st.markdown(
            f"""
            <div class="metric-card fade-in" style="margin-bottom: 0.4rem;">
        ...
        if email or perfil_id:
            chave = f"alerta_acao_{perfil_id or email}_{nome}"
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✉️ Notificar paciente", key=f"notif_{chave}",
                             use_container_width=True):
                    self._notificar_paciente(perfil_id, email, nome, titulo)
            with col2:
                if st.button("👤 Ver paciente →", key=f"ver_{chave}",
                             use_container_width=True):
    ...
    def _notificar_paciente(self, perfil_id: str, email: str, nome: str, titulo: str) -> None:
        """Cria notificação in-app + email para o paciente, a partir do alerta."""
        try:
            pro = st.session_state.get("professional", {})
            pro_nome = pro.name if hasattr(pro, "name") else pro.get("name", "Seu profissional")

            # Notificação in-app
            if self.db.is_real and self.db.client and perfil_id:
                self.db.client.table("notificacoes").insert({
                    "perfil_id": perfil_id,
                    "mensagem": f"💬 {pro_nome} quer falar com você sobre: {titulo}",
                    "tipo": "contato_profissional",
                    "lida": False,
                }).execute()
            ...
            if email:
                try:
                    ...
                    send_clinical_action(email, nome, titulo, f"{pro_nome} registrou: {titulo}")
                except Exception:
                    pass

            st.toast(f"✅ {nome} foi notificado(a)", icon="✉️")
        except Exception as e:
            logger.error(f"_notificar_paciente: {e}")
            st.toast("❌ Erro ao notificar paciente", icon="❌")
