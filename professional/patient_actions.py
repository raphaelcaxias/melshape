...
class PatientActionsRenderer:
    ...
    def _render_tabs(self) -> None:
        """Renderiza as 4 tabs de ações (Sprint 4: +Metas Nutricionais)."""
        tab_conduta, tab_obs, tab_presc, tab_metas = st.tabs([
            "📋 Conduta",
            "📝 Observação",
            "🥗 Prescrição",
            "🎯 Metas Nutricionais",
        ...
        with tab_metas:
            self._render_tab_metas_nutricionais()

    def _render_tab_metas_nutricionais(self) -> None:
        """
        Metas nutricionais personalizáveis pelo profissional.

        Sprint 4 — sobrescreve as fórmulas automáticas (TMB × fator)
        quando o profissional tem protocolo específico para o paciente.

        Salva em `perfis.meta_cal_custom` e `perfis.meta_prot_custom`.
        """
        try:
            st.markdown(f"##### 🎯 Metas Nutricionais — {self.nome}")
            st.caption(
                "Defina metas específicas para este paciente. Quando preenchidas, "
                "substituem o cálculo automático por fórmula (TMB × fator de atividade)."
            )
            ...
            meta_prot_atual = 0.0
            try:
                if self.db.is_real and self.db.client and self.perfil_id:
                    r = (
                        self.db.client.table("perfis")
                        .select("meta_cal_custom,meta_prot_custom")
                        .eq("id", self.perfil_id)
                        .limit(1)
                        .execute()
                    )
                    if r.data:
                        meta_cal_atual = int(r.data[0].get("meta_cal_custom") or 0)
                        meta_prot_atual = float(r.data[0].get("meta_prot_custom") or 0.0)
            except Exception:
                pass
            ...
            with col1:
                meta_cal = st.number_input(
                    "Meta calórica diária (kcal)",
                    min_value=0,
                    max_value=5000,
                    value=meta_cal_atual,
                    step=50,
                    key=f"meta_cal_{self.perfil_id}",
                    help="0 = usar cálculo automático por fórmula",
                )
            with col2:
                meta_prot = st.number_input(
                    "Meta de proteína diária (g)",
                    min_value=0.0,
                    max_value=300.0,
                    value=meta_prot_atual,
                    step=5.0,
                    key=f"meta_prot_{self.perfil_id}",
                    help="0 = usar cálculo automático",
                )
            ...
            st.markdown(
                ...
                <div style="font-size:0.8rem;color:var(--text-muted);
                    margin:.4rem 0 .8rem;">
                    💡 <b>Dica:</b> Deixe 0 para manter o cálculo automático.
                    Valores acima de 0 sobrescrevem completamente a fórmula.
                </div>
                """,
            ...
            if st.button("💾 Salvar Metas", type="primary",
                         ...
                         key=f"salvar_metas_{self.perfil_id}"):
                self._salvar_metas_nutricionais(
                    meta_cal=meta_cal,
                    meta_prot=meta_prot,
                )

        except Exception as e:
            logger.error(f"_render_tab_metas_nutricionais: {e}", exc_info=True)
            alert("❌ Erro ao carregar metas nutricionais.", "error")

    def _salvar_metas_nutricionais(self, meta_cal: int, meta_prot: float) -> None:
        """Salva metas nutricionais customizadas no perfil do paciente."""
        try:
            ...
            if self.db.is_real and self.db.client:
                self.db.client.table("perfis").update({
                    "meta_cal_custom": meta_cal if meta_cal > 0 else None,
                    "meta_prot_custom": meta_prot if meta_prot > 0 else None,
                }).eq("id", self.perfil_id).execute()

                st.toast(
                    f"✅ Metas salvas: {meta_cal} kcal / {meta_prot:.0f}g proteína"
                    if (meta_cal > 0 or meta_prot > 0)
                    else "✅ Metas removidas — cálculo automático ativado",
                    icon="✅",
                )
                st.cache_data.clear()
            else:
                st.info("✅ Metas salvas (modo demo — sem persistência).")
        except Exception as e:
            logger.error(f"_salvar_metas_nutricionais: {e}")
