...
# ─────────────────────────────────────────────────────────────────────────────
# FORMULÁRIOS RÁPIDOS — usados por register_hub.py
# ─────────────────────────────────────────────────────────────────────────────

def _form_agua(db, gami) -> None:
    """Formulário rápido de registro de água."""
    from datetime import date
    from core.models import HydrationLog, HydrationSource

    st.markdown("#### 💧 Registrar Água")
    ml = st.number_input("Quantidade (ml)", min_value=50, max_value=1000, value=200, step=50, key="hub_agua_ml")
    if st.button("✅ Adicionar", type="primary", use_container_width=True, key="hub_agua_btn"):
        try:
            log = HydrationLog(amount_ml=ml, source=HydrationSource.WATER, log_date=date.today().isoformat())
            if db.save_hydration(log):
                st.toast(f"💧 {ml}ml registrados!", icon="✅")
                st.cache_data.clear()
                st.rerun()
            else:
                st.toast("❌ Erro ao registrar água.", icon="❌")
        except Exception as e:
            logger.error(f"_form_agua: {e}")
            st.error("❌ Erro ao registrar água.")


def _form_checkin(db, gami) -> None:
    """Formulário rápido de check-in."""
    st.markdown("#### ✅ Check-in Rápido")
    humor = st.select_slider("😊 Humor", options=[1, 2, 3, 4, 5], value=3, key="hub_ci_humor")
    if st.button("✅ Salvar check-in", type="primary", use_container_width=True, key="hub_ci_btn"):
        try:
            if db.save_checkin(humor, 3, 3.0):
                st.toast("✅ Check-in salvo!", icon="✅")
                st.cache_data.clear()
                st.rerun()
            else:
                st.toast("❌ Erro ao salvar check-in.", icon="❌")
        except Exception as e:
            logger.error(f"_form_checkin: {e}")
